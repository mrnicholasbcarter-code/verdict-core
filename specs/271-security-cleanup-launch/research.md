...
- Add a new security scanner or runtime service: rejected because it would
  duplicate existing CI ownership and exceed issue scope.
- Record a launch approval locally without hosted evidence: rejected because
  acceptance requires evidence-bound review and CI state.

## Decision

EXTEND the existing workflow/checklist contracts and protect them with tests.
The previous vulnerability exceptions were removed because they made dependency
review advisory. Any upstream vulnerability stays visible as a blocking
limitation rather than being silently accepted.
