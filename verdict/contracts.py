...
def _validate_field_value(annotation: Any, value: Any, field_name: str) -> None:
    ...
    if _is_contract_type(annotation):
        if isinstance(value, annotation):
            return
        if isinstance(value, dict):
            # Validate the nested contract payload now; ``_coerce_field_value``
            # converts it to a Contract instance after validation succeeds.
...
def _validate_contract_semantics(cls: type[Contract], payload: dict[str, Any]) -> None:
    ...
    elif cls is VerificationPlan:
        _validate_enum(
            payload.get("on_failure", "deny"), frozenset({"deny", "replan_or_deny"}), "on_failure"
        )
