...
from verdict.availability import AvailabilityReport, AvailabilityState, CandidateRequirements, OmniRouteAvailabilityAdapter
...
class SubagentModelSelector:
    ...
    def from_environment(cls) -> SubagentModelSelector:
        ...
        base_url = os.getenv("OMNIROUTE_BASE_URL") or os.getenv("LLMGATE_UPSTREAM_BASE_URL", "http://127.0.0.1:20128/v1")
    ...
    def select_for_role(
        ...
        *,
        ...
        if requirements is None:
            raise ValueError(f"Unknown role: {role}. Valid: {list(SUBAGENT_ROLE_REQUIREMENTS.keys())}")
        ...
        def fast_lookup(model_id: str) -> AvailabilityReport:
            ...
            if c is None:
                return AvailabilityReport(
                    candidates=(), eligible=(), source='lookup',
                    freshness_seconds=None, errors=('not found',)
                )
            state = c.state
            admitted_states = {AvailabilityState.ELIGIBLE, AvailabilityState.READY, AvailabilityState.DEGRADED}
            eligible = (c,) if state in admitted_states else ()
            return AvailabilityReport(
                candidates=(c,), eligible=eligible, source=c.source,
                freshness_seconds=report.freshness_seconds, errors=report.errors
            )
        ...
        eligible_result = fast_gate.evaluate(
            [c.model for c in report.candidates],
            protected=protected,
            dev_mode=dev_mode,
        )
        ...
        task_spec = type("TaskSpec", (), {
            ...
            "context": {"role": role},
            "requirements": [],
            "budget_per_1k": requirements.estimated_cost / max(1, requirements.estimated_tokens / 1000) if requirements.estimated_tokens else None,
    ...
    def select_for_parallel_roles(
        ...
        *,
        ...
        role_order = sorted(roles, key=lambda r: SUBAGENT_ROLE_REQUIREMENTS[r].budget_remaining or 0, reverse=True)
...
def select_model_for_role(
    ...
    *,
    ...
    return selector.select_for_role(role, protected=protected, dev_mode=dev_mode, diversity_from=diversity_from)
