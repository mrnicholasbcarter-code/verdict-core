...
class TestCapabilityManifest:
    ...
    def test_manifest_to_from_dict(self):
        manifest = CapabilityManifest(
            required=["a", "b"], optional=["c"], forbidden=["d"], minimum_versions={"a": "1.0"}
        )
...
class TestRequestResponseEnvelopes:
    ...
    def test_submit_request_envelope(self):
        ...
        request = RufloSubmitRequest(
            task_spec=task_spec.to_dict(), budget_usd=5.0, priority=10, metadata={"source": "test"}
        )
    ...
    def test_status_request_response_envelope(self):
        request = RufloStatusRequest(
            task_id="task-123", include_history=True, include_verification=True
        )
    ...
    def test_control_request_response_envelope(self):
        request = RufloControlRequest(task_id="task-123", action=TaskAction.PAUSE, reason="Testing")
...
class TestFakeAdapter:
    ...
    def test_fake_submit_with_workflow(self, fake_adapter):
        ...
        response = fake_adapter.submit(
            task_spec=task_spec, workflow_plan=workflow_plan, budget_usd=10.0
        )
...
class TestIntegrationWithContracts:
    ...
    def test_submit_with_task_spec_and_workflow_plan(self):
        ...
        response = adapter.submit(task_spec=task_spec, workflow_plan=workflow_plan, budget_usd=10.0)
...
class TestConfigDefaults:
    ...
    def test_custom_config(self):
        config = RufloAdapterConfig(fake_mode=True, fake_latency_ms=50, trust_protected_work=False)
