...
class TestIntelligenceAdapterConfig:
    ...
    def test_custom_config(self):
        config = IntelligenceAdapterConfig(
            profile="degraded", allow_degraded_mode=True, planner_timeout_ms=10000
        )
...
class TestIntelligenceAdapter:
    ...
    def mock_planner(self):
        ...
        planner.plan.return_value = Mock(
            task_spec=TaskSpec(objective="test task", task_type="code"), workflow_plan=None
        )
        return planner
    ...
    def test_successful_execution(self, adapter, mock_planner):
        ...
        result = adapter.execute(
            objective="Write a hello world function", criticality="low", budget={"max_usd": 0.10}
        )
...
class TestCategorizedFailureErrors:
    ...
    def adapter_with_failing_planner(self):
        ...
        return IntelligenceAdapter(
            config=IntelligenceAdapterConfig(profile="development"), planner=planner
        )

    def test_plan_rejected_categorized(self, adapter_with_failing_planner):
        """Test plan rejection is categorized as PLAN_FAILURE."""
        with pytest.raises(CategorizedFailureError) as exc_info:
...
class TestRedaction:
    ...
    def test_redacts_api_keys(self):
        adapter = IntelligenceAdapter(config=IntelligenceAdapterConfig(redact_secrets=True))
    ...
    def test_redaction_disabled(self):
        adapter = IntelligenceAdapter(config=IntelligenceAdapterConfig(redact_secrets=False))
...
class TestFactoryFunction:
    ...
    def test_factory_with_custom_components(self):
        ...
        adapter = build_intelligence_adapter(planner=planner, eligibility_gate=gate)
...
class TestErrorCategories:
    ...
    def test_categorized_failure_category(self):
        err = CategorizedFailureError("plan failed", failure_class="PLAN_FAILURE")
    ...
    def test_categorized_failure_replannable(self):
        err = CategorizedFailureError("plan failed", failure_class="PLAN_FAILURE", replannable=True)
